...
$user_authentication_query = "select id, name, email, contact, city, address, status from users where email='$email' and password='$password'";
...
if ($rows_fetched == 0) {
    ...
    $_SESSION['name'] = $row['name']; //status user
    $_SESSION['status'] = $row['status']; //status user
    $_SESSION['contact'] = $row['contact']; //contact user
